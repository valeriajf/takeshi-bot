/**
 * INSTALAÇÃO: src/commands/admin/set-image-grupo-abrir.js
 */
...
export default {
  name: "set-image-grupo-abrir",
  description: "Define a imagem enviada junto com a mensagem automática de abertura do grupo.",
  commands: ["set-image-grupo-abrir", "imagem-abrir"],
  usage: `${PREFIX}set-image-grupo-abrir (envie com uma imagem)\n${PREFIX}set-image-grupo-abrir remover`,
  ...
  handle: async ({
    ...
    if (args[0]?.toLowerCase() === "remover") {
      ...
      await sendSuccessReply("✅ Imagem de abertura removida com sucesso!");
      return;
    }
    ...
    if (!imageMsg) {
      throw new DangerError(
        ...
        `💡 Para GIF use: ${PREFIX}set-gif-grupo-abrir`
      );
    }
    ...
    await sendSuccessReply(
      `✅ *Imagem de abertura definida!*\n\n` +
      `🖼️ Esta imagem será enviada automaticamente junto com a mensagem de abertura.\n\n` +
      `⚠️ GIF anterior removido automaticamente.\n\n` +
      `Para remover: ${PREFIX}set-image-grupo-abrir remover`
    );
  },
};
