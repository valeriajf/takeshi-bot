/**
 * INSTALAÇÃO: src/commands/admin/set-gif-grupo-abrir.js
 */
...
export default {
  name: "set-gif-grupo-abrir",
  description: "Define o GIF enviado junto com a mensagem automática de abertura do grupo.",
  commands: ["set-gif-grupo-abrir", "gif-abrir"],
  usage: `${PREFIX}set-gif-grupo-abrir (envie com um GIF)\n${PREFIX}set-gif-grupo-abrir remover`,
  ...
  handle: async ({
    ...
    if (args[0]?.toLowerCase() === "remover") {
      ...
      save(GIF_FILE, gifs);
      await sendSuccessReply("✅ GIF de abertura removido com sucesso!");
      return;
    }
    ...
    if (!videoMsg || !videoMsg.gifPlayback) {
      throw new DangerError(
        ...
        `💡 Para imagem estática use: ${PREFIX}set-image-grupo-abrir`
      );
    }
    ...
    await sendSuccessReply(
      `✅ *GIF de abertura definido!*\n\n` +
      `🎬 Este GIF será enviado automaticamente junto com a mensagem de abertura.\n\n` +
      `⚠️ Imagem anterior removida automaticamente.\n\n` +
      `Para remover: ${PREFIX}set-gif-grupo-abrir remover`
    );
  },
};
